...
export default function IdeaCard({
    title = "AI 도출 사업 컨셉",
    concept = "생각 정리 부담을 해소하는 시각적 마인드맵 기반 AI 사업계획서 자동 생성 SaaS",
    feedback = "현재 '팀 구성(Team)' 관련 정보가 다소 부족합니다. 개발 및 사업화 인력 역량을 추가하면 계획서 완성도가 더 높아집니다.",
}: IdeaCardProps) {
    return (
        <div className="relative">
            ...
            <Card className="w-64 gap-3 p-3 shadow-md bg-card">
                <span className="font-semibold text-neutral-400">{title}</span>
                <div className="flex flex-col gap-1">
                    <Separator />
                    <Separator />
                </div>
                <p className="text-justify text-xs">{concept}</p>
